export const scheduleTbdNote = `Schedule TBD — we're still working out the details.`

export const headlineWords = ['games,', 'apps,', 'projects,', 'stuff,']

export const doList = [
  {
    title: 'Hack Club',
    body: 'YSWS — "you ship, we ship." Build something, get it shipped, and earn rewards.',
    icon: 'rocket'
  },
  {
    title: 'Hackathon teams',
    body: 'Form teams with people in this club and compete at hackathons around the Bay Area. Practice at club-hosted mock hackathons.',
    icon: 'users'
  },
  {
    title: 'Mini projects',
    body: 'Small projects built in one lunch period that you can use.',
    icon: 'wrench'
  }
]

export const facts = [
  {
    title: 'Biweekly meetings',
    body: 'Workshops & build sessions. Build fun, unique projects of your choice.'
  },
  {
    title: 'Honor Hall',
    body: 'Get added to the Honor Hall for shipping, mentoring, and teaching. And increase your chances at board positions...'
  },
  {
    title: 'After-school events',
    body: 'Attend hackathons, club-only mock hackathons (small prizes...), or after school workshops.'
  },
  {
    title: 'Hack Club',
    body: "We're a Hack Club chapter — hardware grants and hackathon travel."
  }
]

export const sponsors: { name: string; url?: string }[] = []

export const socials = [
  { name: 'Discord', icon: 'discord', url: 'https://discord.gg/QWVXym8pw6', color: '#5865F2' },
  { name: 'Instagram', icon: 'instagram', url: 'https://www.instagram.com/dublinhighcs', color: '#E4405F' },
]

export const contactEmail = 'dublincomputerscienceclub@gmail.com'

export const steps = [
  { n: '1', title: 'We email you once', body: 'A short note with the next meeting date and where D-204 actually is.' },
  { n: '2', title: 'Show up, sit anywhere', body: 'First-timers get paired with an officer for the build half of the meeting.' },
  { n: '3', title: "You're a member", body: 'No vote, no dues. Come to two meetings and you\'re on the roster.' }
]

export const faq = [
  { q: 'Do I need to know how to code?', a: 'No. About half of our members joined having written nothing but Scratch.' },
  { q: 'Do I have to come to every meeting?', a: "No. Biweekly means it's easy to keep up, and every session stands alone." },
  { q: 'What is Hack Club?', a: 'A network of student-run coding clubs. Being a chapter gets us hackathon travel, hardware grants, and a Slack full of other high schoolers.' }
]

export const grades = ['9th', '10th', '11th', '12th']

export const levels = [
  { label: 'never written code', hint: 'most common answer' },
  { label: 'a little from class', hint: 'ap csp, scratch, khan' },
  { label: 'i build things already', hint: 'bring a project' }
]

export const interests = [
  'web apps', 'game dev', 'robotics', 'ai / ml',
  'hardware', 'competitive programming', 'design', 'cybersecurity'
]
